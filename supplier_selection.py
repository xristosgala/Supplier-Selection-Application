...
def solve_supplier_selection_problem(num_weeks, w1, w2, w3, num_suppliers, costs, lead_times, quality_scores, 
    ...
    results = {
        ...
        "Weekly Demand": weekly_demand.tolist(),
        "Details": []
    }
...
costs = {supplier: st.sidebar.number_input(f"Cost for {supplier}", min_value=1, max_value=100, value=10) for supplier in suppliers}
lead_times = {supplier: st.sidebar.number_input(f"Lead Time for {supplier}", min_value=1, max_value=100, value=12) for supplier in suppliers}
quality_scores = {supplier: st.sidebar.number_input(f"Quality Score for {supplier}", min_value=1, max_value=100, value=8) for supplier in suppliers}
capacities = {supplier: st.sidebar.number_input(f"Capacity for {supplier}", min_value=1, max_value=1000, value=100) for supplier in suppliers}
min_order = {supplier: st.sidebar.number_input(f"Minimum Order for {supplier}", min_value=10, max_value=100, value=10) for supplier in suppliers}
